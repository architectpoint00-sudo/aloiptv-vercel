...
export default function ComparisonTable({ rows }: ComparisonTableProps) {
  return (
    <div className="rounded-2xl overflow-hidden bg-[#111827] border border-[#1e293b]">
      {/* Table Header */}
      <div className="grid grid-cols-3 bg-[#0d1224]">
      ...
      {rows.map((row, index) => (
        <div
          key={index}
          className={`grid grid-cols-3 border-t border-[#1e293b] hover:bg-white/[0.02] transition-colors ${
        ...
        >
          ...
          <div className="px-4 sm:px-6 py-4 flex items-center justify-center gap-2">
            ...
            <span className="text-xs sm:text-sm font-semibold text-green-400">{row.aloiptv}</span>
          </div>
          <div className="px-4 sm:px-6 py-4 flex items-center justify-center gap-2">
